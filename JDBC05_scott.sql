...
-- 전체 출력 - 사번 정렬(기본)
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, NVL(BASICPAY, 0) + NVL(SUDANG, 0) AS SAL
...
-- 전체 출력 - 이름 정렬
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, NVL(BASICPAY, 0) + NVL(SUDANG, 0) AS SAL FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID ORDER BY EMP_NAME ASC
;
...
-- 전체 출력 - 급여 내림차순 정렬
SELECT EMP_ID, EMP_NAME, SSN, TO_DATE(IBSADATE, 'YYYY-MM-DD') AS IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, NVL(BASICPAY, 0) + NVL(SUDANG, 0) AS SAL  
FROM TBL_EMP E JOIN TBL_CITY C
ON E.CITY_ID = C.CITY_ID
JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID
JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID
ORDER BY NVL(BASICPAY, 0) + NVL(SUDANG, 0) DESC;

-- 검색 출력
SELECT EMP_ID, EMP_NAME, SSN, IBSADATE, CITY_NAME, TEL, BUSEO_NAME, JIKWI_NAME, BASICPAY, SUDANG, NVL(BASICPAY, 0) + NVL(SUDANG, 0) AS SAL FROM TBL_EMP E JOIN TBL_CITY C ON E.CITY_ID = C.CITY_ID JOIN TBL_BUSEO B ON E.BUSEO_ID = B.BUSEO_ID JOIN TBL_JIKWI J ON E.JIKWI_ID = J.JIKWI_ID WHERE BUSEO_NAME = '인사부'
;
...
-- 데이터 수정
UPDATE TBL_EMP SET EMP_NAME='%s', SSN='%s', IBSADATE=TO_DATE('%s', 'YYYY-MM-DD'), CITY_ID=%d, TEL ='%s', BUSEO_ID=%d, JIKWI_ID=%d, BASICPAY=%d, SUDANG=%d WHERE EMP_ID = %d
;
...
SELECT CITY_NAME FROM TBL_CITY WHERE CITY_ID = 1
;

-- 기본급 출력
SELECT * FROM TBL_JIKWI WHERE JIKWI_NAME = ;
...
1062	이호석	961112-1234567	2020-11-11	서울	010-9600-1715	개발부	대리	5000000	2000000	7000000
...
1061	최문정	990702-2150115	2022-03-24	인천	010-2877-0992	개발부	사원	2000000	1000000	3000000
